...
void RenderVisitor::_visit(Node *node) {
  ...
  if (g_node) {
    ...
    const GLfloat *data = g_node->getVBO();
    const GLuint *indices = g_node->getEBO();
    ...
    glBindVertexArray(VAO); {
      ...
      glBufferData(GL_ARRAY_BUFFER, g_node->getDataSize() / sizeof(GLfloat), data, GL_STATIC_DRAW);
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
      glBufferData(GL_ELEMENT_ARRAY_BUFFER, g_node->getIndicesSize() / sizeof(GLuint), indices,
          GL_STATIC_DRAW);
